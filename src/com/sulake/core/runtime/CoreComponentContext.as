package com.sulake.core.runtime
{
   ...
   public final class CoreComponentContext extends ComponentContext implements ICore
   {
      ...
      private static const const_110:uint = 3;
      
      private static const const_1329:String = "asset-libraries";
      
      private static const const_1326:String = "library";
      
      private static const const_1325:String = "service-libraries";
      
      private static const const_1322:String = "library";
      
      private static const const_1324:String = "component-libraries";
      
      private static const const_1321:String = "library";
      
      private static const const_1323:String = "error_data";
      
      private static const const_1327:String = "error_cat";
      
      private static const const_1328:String = "error_desc";
       
      
      private var var_343:LibraryLoaderQueue;
      ...
      public function CoreComponentContext(param1:DisplayObjectContainer, param2:uint)
      {
         ...
         var_402 = param1;
         var _loc3_:int = 0;
         while(_loc3_ < CoreComponentContext.const_110)
         {
            this.var_245.push(new Array());
            this.var_587.push(0);
            _loc3_++;
         }
         ...
         switch(param2 & 0)
         {
            ...
            case Core.const_763:
               ...
               this.var_199 = new Profiler(this);
               attachComponent(this.var_199,[new IIDProfiler()]);
               var_402.addChild(new ProfilerViewer(this.var_199));
               break;
            case Core.const_491:
      ...
      {
         return this.var_343.length;
      }
      
      public function getNumberOfFilesLoaded() : uint
      {
         return this.var_490 - this.getNumberOfFilesPending();
      }
      ...
      override public function dispose() : void
      {
         ...
         if(!disposed)
         {
            ...
            {
               i = 0;
               while(i < CoreComponentContext.const_110)
               {
                  receivers = this.var_245[i] as Array;
      ...
      override public function error(param1:String, param2:Boolean, param3:int = -1, param4:Error = null) : void
      {
         if(param4)
         {
            ErrorReportStorage.setParameter(const_1323,String(param4.getStackTrace()));
         }
         ErrorReportStorage.setParameter(const_1327,String(param3));
         ErrorReportStorage.setParameter(const_1328,param1);
      ...
      public function readConfigDocument(param1:XML, param2:IEventDispatcher = null) : void
      {
         ...
         this.var_720 = loadingEventDelegate;
         if(this.var_343 == null)
         {
            this.var_343 = new LibraryLoaderQueue(_debug);
         }
         try
         {
            node = config.child(const_1329)[0];
            if(node != null)
            {
         ...
         {
            error("Failed to parse asset libraries from config xml!",true,Core.const_323,e);
         }
         try
         {
         ...
         {
            error("Failed to parse interfaces from config xml!",true,Core.const_323,e);
         }
         try
         {
            node = config.child(const_1324)[0];
            if(node != null)
            {
         ...
         {
            error("Failed to parse components from config xml!",true,Core.const_323,e);
         }
         if(!disposed)
         {
      ...
      private function errorInLoadingProcess(param1:LibraryLoaderEvent = null) : void
      {
         ...
         this.error("Failed to download library \"" + _loc2_.url + "\" HTTP status " + param1.status + ": " + _loc2_.getLastErrorMessage(),true,Core.const_1680);
         if(!disposed)
         {
      ...
      private function finalizeLoadingEventDelegate() : void
      {
         if(this.var_720)
         {
            this.var_720.dispatchEvent(new Event(Event.COMPLETE));
         }
      }
      ...
      private function updateLoadingProgress(param1:LibraryLoaderEvent = null) : void
      {
         ...
         if(this.var_720)
         {
            ...
            this.var_720.dispatchEvent(new LibraryProgressEvent(_loc2_.url,param1.bytesLoaded,param1.bytesTotal,_loc2_.elapsedTime));
         }
      }
         ...
         if(param1 != null)
         {
            if(param1.type == LibraryLoaderEvent.LIBRARY_LOADER_EVENT_COMPLETE || param1.type == LibraryLoaderEvent.LIBRARY_LOADER_EVENT_ERROR)
            {
               ...
               if(!disposed)
               {
                  if(this.var_720)
                  {
                     this.var_720.dispatchEvent(new LibraryProgressEvent(_loc2_.url,this.var_490 - this.var_343.length,this.var_490,_loc2_.elapsedTime));
                  }
               }
            }
         }
      ...
      override public function registerUpdateReceiver(param1:IUpdateReceiver, param2:uint) : void
      {
         ...
         param2 = param2 >= CoreComponentContext.const_110 ? 4294967295 : uint(param2);
         var _loc3_:int = !!this.var_199 ? 0 : this.var_1375 & 0;
         if(_loc3_ == Core.const_491)
         {
            this.var_245[param2].push(new UpdateDelegate(param1,this,param2));
         }
         else
         {
            this.var_245[param2].push(param1);
         }
      }
      ...
      override public function removeUpdateReceiver(param1:IUpdateReceiver) : void
      {
         ...
         var _loc3_:* = null;
         var _loc6_:* = null;
         var _loc4_:int = !!this.var_199 ? 0 : this.var_1375 & 0;
         var _loc5_:int = 0;
         while(_loc5_ < CoreComponentContext.const_110)
         {
            _loc3_ = this.var_245[_loc5_] as Array;
            if(_loc4_ == Core.const_491)
            {
      ...
      private function simpleFrameUpdateHandler(param1:uint, param2:uint) : void
      {
         ...
         priority = 0;
         while(priority < CoreComponentContext.const_110)
         {
            this.var_587[priority] = 0;
            receivers = this.var_245[priority];
            index = 0;
            length = receivers.length;
            while(index != length)
            {
      ...
      private function complexFrameUpdateHandler(param1:uint, param2:uint) : void
      {
         ...
         while(priority < CoreComponentContext.const_110)
         {
            ...
            {
               if(this.var_587[priority] < priority)
               {
                  ++this.var_587[priority];
                  skip = true;
               }
            }
            if(!skip)
         ...
         while(priority < CoreComponentContext.const_110)
         {
            ...
            while(index != length)
            {
               ...
               else
               {
                  try
                  {
                     this.var_199.update(receiver,msSinceLastUpdate);
                  }
                  catch(e:Error)
                  {
                     error("Error in update receiver \"" + getQualifiedClassName(receiver) + "\": " + e.message,true,e.errorID,e);
      ...
      public function setProfilerMode(param1:Boolean) : void
      {
         ...
         if(param1)
         {
            ...
            _loc2_ = 0;
            while(_loc2_ < CoreComponentContext.const_110)
            {
         ...
         else
         {
            if(this.var_199)
            {
               detachComponent(this.var_199);
               this.var_199.dispose();
               this.var_199 = null;
            }
            switch(this.var_1375 & 0)
            {
               case Core.const_1092:
                  this.var_344 = this.simpleFrameUpdateHandler;
                  break;
