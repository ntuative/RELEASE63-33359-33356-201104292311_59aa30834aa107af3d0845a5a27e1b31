package com.sulake.core.window.utils
{
   ...
   public class MouseEventProcessor implements IEventProcessor, IDisposable
   {
      ...
      protected var var_877:IWindowContextStateListener;
      ...
      public function MouseEventProcessor()
      {
         ...
         {
            var_332 = new Array();
            var_332[0] = MouseCursorType.const_324;
            var_332[1] = MouseCursorType.const_29;
            var_332[2] = MouseCursorType.const_324;
            var_332[3] = MouseCursorType.const_324;
            var_332[4] = MouseCursorType.const_324;
            var_332[5] = MouseCursorType.const_29;
            var_332[6] = MouseCursorType.const_324;
         }
         if(var_287 == null)
         {
            var_287 = new Array();
            var_287[0] = WindowState.const_109;
            var_287[1] = WindowState.const_88;
            var_287[2] = WindowState.const_85;
            var_287[3] = WindowState.const_68;
            var_287[4] = WindowState.const_54;
            var_287[5] = WindowState.const_92;
      ...
      public function process(param1:EventProcessorState, param2:IEventQueue) : void
      {
         ...
         this.var_62 = state.var_1329 as WindowController;
         this.var_173 = state.var_1328 as WindowController;
         this.var_166 = state.renderer;
         this.var_877 = state.var_1326;
         eventQueue.begin();
         this.var_707.x = -1;
         this.var_707.y = -1;
         ...
         while(true)
         {
            ...
            if(event.stageX != this.var_707.x || event.stageY != this.var_707.y)
            {
               ...
               array = new Array();
               this.var_81.method_10(this.var_707,array,WindowParam.const_30);
            }
            ...
            if(index == 0)
            {
               if(event.type == MouseEvent.MOUSE_MOVE)
               {
                  ...
                  {
                     this.var_62.getGlobalPosition(var_405);
                     tempWindowEvent = WindowMouseEvent.allocate(WindowMouseEvent.WINDOW_EVENT_MOUSE_OUT,this.var_62,null,event.stageX - 0,event.stageY - 0,event.stageX,event.stageY,event.altKey,event.ctrlKey,event.shiftKey,event.buttonDown,event.delta);
               ...
               if(child != null && child.visible)
               {
                  if(event.type == MouseEvent.MOUSE_MOVE)
                  {
                     if(child != this.var_62)
                     {
                        ...
                        {
                           this.var_62.getGlobalPosition(var_405);
                           tempWindowEvent = WindowMouseEvent.allocate(WindowMouseEvent.WINDOW_EVENT_MOUSE_OUT,this.var_62,child,event.stageX - 0,event.stageY - 0,event.stageX,event.stageY,event.altKey,event.ctrlKey,event.shiftKey,event.buttonDown,event.delta);
                           this.var_62.update(this.var_62,tempWindowEvent);
                           tempWindowEvent.recycle();
                        }
                        if(!child.disposed)
                  ...
                  else if(event.type == MouseEvent.MOUSE_UP || event.type == MouseEvent.CLICK)
                  {
                     if(this.var_62 && !this.var_62.disposed)
                     {
                        ...
                        {
                           this.var_877.mouseEventReceived(event.type,this.var_62);
                        }
                     }
                  }
                  temp = child.parent;
         ...
         state.var_1329 = this.var_62;
         state.var_1328 = this.var_173;
         state.renderer = this.var_166;
         state.var_1326 = this.var_877;
      }
      ...
      private function passMouseEvent(param1:WindowController, param2:MouseEvent, param3:Boolean = false) : WindowController
      {
         ...
         if(param1.testParamFlag(WindowParam.const_599))
         {
