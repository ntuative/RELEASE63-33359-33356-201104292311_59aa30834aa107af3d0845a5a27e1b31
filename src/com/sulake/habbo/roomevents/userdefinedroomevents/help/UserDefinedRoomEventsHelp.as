package com.sulake.habbo.roomevents.userdefinedroomevents.help
{
   ...
   public class UserDefinedRoomEventsHelp
   {
      ...
      private function prepareWindow() : void
      {
         ...
         this.var_1798 = IWindowContainer(this.find(this._window,"help_container"));
         this.var_1177 = this.var_1798.findChildByName("button_previous") as IContainerButtonWindow;
         if(this.var_1177)
         {
            this.var_1177.procedure = this.onButtonClick;
         }
         this.var_1178 = this.var_1798.findChildByName("button_next") as IContainerButtonWindow;
         if(this.var_1178)
         {
            this.var_1178.procedure = this.onButtonClick;
         }
      ...
      {
         if(this.var_250 >= this.var_58.length - 1)
         {
            this.var_1178.disable();
         }
         else
         {
            this.var_1178.enable();
         }
         if(this.var_250 <= 0)
         {
            this.var_1177.disable();
         }
         else
         {
            this.var_1177.enable();
         }
      }
      ...
      {
         ++this.var_250;
         this.var_250 = Math.min(this.var_250,this.var_58.length - 1);
         this.openPage();
      }
      ...
      {
         --this.var_250;
         this.var_250 = Math.max(this.var_250,0);
         this.openPage();
      }
      ...
      private function initLocalizations() : void
      {
         ...
         while(this.var_80.localization.getLocalization("wiredfurni.help." + _loc1_ + ".title"))
         {
            ...
            this.var_58.push([_loc2_,_loc3_,_loc4_]);
            _loc1_++;
         }
      }
      ...
      private function localize() : void
      {
         ...
         if(!this.var_58 || this.var_58.length == 0)
         {
            return;
         }
         if(_loc1_)
         {
            _loc1_.text = ILocalization(this.var_58[this.var_250][0]).value;
         }
         if(_loc2_)
         {
            _loc2_.text = ILocalization(this.var_58[this.var_250][2]).value;
         }
         if(_loc3_)
         {
            _loc4_ = ILocalization(this.var_58[this.var_250][1]).value;
