package com.sulake.habbo.avatar.common
{
   ...
   public class AvatarEditorGridColorItem
   {
      ...
      private var var_957:IPartColor;
      ...
      private function updateThumbData() : void
      {
         ...
         if(_loc1_ != null)
         {
            _loc1_.bitmap = new BitmapData(this.var_651.width,this.var_651.height,true,0);
            _loc1_.bitmap.copyPixels(this.var_651,this.var_651.rect,new Point(0,0));
         }
         var _loc2_:IBitmapWrapperWindow = this._window.findChildByTag("COLOR_IMAGE") as IBitmapWrapperWindow;
         if(_loc2_ != null)
         {
            _loc2_.bitmap = new BitmapData(this.var_992.width,this.var_992.height,true,0);
            _loc5_ = this.var_992.clone();
            _loc5_.colorTransform(_loc5_.rect,this.var_957.colorTransform);
         ...
         if(_loc3_ != null)
         {
            if(this._isSelected)
            {
               _loc3_.bitmap = new BitmapData(this.var_958.width,this.var_958.height,true,16777215);
               _loc3_.bitmap.copyPixels(this.var_958,this.var_958.rect,new Point(0,0),null,null,true);
               _loc3_.visible = true;
            }
            else
            {
         ...
         if(_loc4_)
         {
            if(this.var_957)
            {
               switch(this.var_957.clubLevel)
               {
                  case HabboClubLevelEnum.const_52:
                     _loc4_.visible = false;
                     break;
                  case HabboClubLevelEnum.const_44:
                     _loc4_.style = HabboIconType.const_918;
                     _loc4_.visible = true;
                     break;
                  case HabboClubLevelEnum.const_45:
                     _loc4_.style = HabboIconType.const_750;
                     _loc4_.visible = true;
               }
            }
            else
