package com.sulake.habbo.avatar.common
{
   ...
   public class AvatarEditorGridPartItem implements IAvatarImageListener
   {
      ...
      private var var_1408:IWindow;
      
      private var var_836:IFigurePartSet;
      ...
      private var var_671:Rectangle;
      
      private var var_2009:int = 0;
      
      private var var_457:IAvatarRenderManager;
      
      private var var_1083:Boolean;
      
      private var var_2007:BitmapData;
      
      private const const_2114:int = 13421772;
      
      private const const_2115:int = 16777215;
      
      private const const_369:Array = [2,6,0,4,3,1];
      ...
      public function AvatarEditorGridPartItem(param1:IWindowContainer, param2:IAvatarEditorCategoryModel, param3:IFigurePartSet, param4:Array, param5:Boolean = true)
      {
         ...
         this.var_836 = param3;
         this._window = param1;
         this.var_1408 = this._window.findChildByTag("BG_COLOR");
         this._colors = param4;
         ...
         if(param3 != null)
         {
            ...
            {
               this.var_2009 = Math.max(this.var_2009,_loc6_.colorLayerIndex);
            }
         }
         this.var_457 = this.var_67.controller.avatarRenderManager;
      ...
      private function updateThumbVisualization() : void
      {
         ...
         if(_loc1_)
         {
            if(this.var_699 != null && !this.var_2008)
            {
               _loc3_ = this.var_699;
            }
            else
         ...
         if(_loc2_)
         {
            if(this.var_836)
            {
               switch(this.var_836.clubLevel)
               {
                  case HabboClubLevelEnum.const_52:
                     _loc2_.visible = false;
                     break;
                  case HabboClubLevelEnum.const_44:
                     _loc2_.style = HabboIconType.const_918;
                     break;
                  case HabboClubLevelEnum.const_45:
                     _loc2_.style = HabboIconType.const_750;
               }
            }
            else
            {
         ...
         if(this.isSelected)
         {
            this.var_1408.color = this.const_2114;
         }
         else
         {
            this.var_1408.color = this.const_2115;
         }
      ...
      private function analyzePartLayers() : Boolean
      {
         ...
         var _loc1_:IAvatarFigureContainer = this.var_457.createFigureContainer(this.partSet.type + "-" + this.partSet.id);
         Logger.log("QUERYING SET:" + this.partSet.type + "-" + this.partSet.id);
         if(!this.var_457.isFigureReady(_loc1_))
         {
            this.var_457.downloadFigure(_loc1_,this);
            return false;
         }
         var _loc6_:int = 0;
         ...
         for each(_loc2_ in this.partSet.parts)
         {
            if(_loc7_)
            {
               _loc3_ = "undefined_undefined_" + _loc2_.type + "_" + _loc2_.id + "_" + this.const_369[_loc6_] + "_" + FigureData.const_150;
               _loc4_ = this.var_457.getAssetByName(_loc3_) as BitmapDataAsset;
            }
            else
            {
               _loc6_ = 0;
               while(!_loc7_ && _loc6_ < this.const_369.length)
               {
                  _loc3_ = "undefined_undefined_" + _loc2_.type + "_" + _loc2_.id + "_" + this.const_369[_loc6_] + "_" + FigureData.const_150;
                  _loc4_ = this.var_457.getAssetByName(_loc3_) as BitmapDataAsset;
      ...
      private function renderThumb() : BitmapData
      {
         ...
         if(this.var_671 == null)
         {
            if(!this.analyzePartLayers())
            {
               if(!this.var_2007)
               {
                  ...
                  this.var_2007 = _loc9_.content as BitmapData;
               }
         ...
         _loc1_ = new BitmapData(this.var_671.width,this.var_671.height,true,16777215);
         var _loc6_:int = 0;
         var _loc7_:Boolean = false;
         ...
         for each(_loc2_ in _loc8_)
         {
            ...
            if(_loc4_)
            {
               ...
               _loc10_ = -1 * _loc4_.offset.x - this.var_671.x;
               _loc11_ = -1 * _loc4_.offset.y - this.var_671.y;
               if(this.var_2008 && _loc2_.colorLayerIndex > 0)
               {
