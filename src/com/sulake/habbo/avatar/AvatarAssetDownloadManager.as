package com.sulake.habbo.avatar
{
   ...
   public class AvatarAssetDownloadManager extends EventDispatcher
   {
      ...
      private var var_911:int = 3;
      
      private var var_419:AssetLoaderStruct;
      ...
      private const const_2133:int = 100;
      
      private const const_1672:int = 2;
      ...
      public function AvatarAssetDownloadManager(param1:IAssetLibrary, param2:String, param3:String, param4:AvatarStructure)
      {
         ...
         if(_loc6_ == null)
         {
            this.var_419 = this._assets.loadAssetFromFile("figuremap",_loc5_,"text/xml");
         ...
         this.var_910 = new Timer(this.const_2133,1);
         this.var_910.addEventListener(TimerEvent.TIMER_COMPLETE,this.onNextDownloadTimeout);
      }
      ...
      private function addMapLoaderEventListeners() : void
      {
         if(this.var_419)
         {
            this.var_419.addEventListener(AssetLoaderEvent.const_28,this.onConfigurationComplete);
            this.var_419.addEventListener(AssetLoaderEvent.const_47,this.onConfigurationError);
         }
      }
      ...
      private function removeMapLoaderEventListeners() : void
      {
         if(this.var_419)
         {
            this.var_419.removeEventListener(AssetLoaderEvent.const_28,this.onConfigurationComplete);
            this.var_419.removeEventListener(AssetLoaderEvent.const_47,this.onConfigurationError);
         }
      }
      ...
      private function onConfigurationError(param1:Event) : void
      {
         ...
         {
            Logger.log("Could not download figuremap.xml. Retries left: " + this.var_911);
            if(this.var_1388.indexOf("?") > 0)
            {
               _loc2_ = this.var_1388 + "&retry=" + this.var_911;
            }
            else
            {
               _loc2_ = this.var_1388 + "?retry=" + this.var_911;
            ...
            this.var_419 = this._assets.loadAssetFromFile("figuremap",_loc3_,"text/xml");
            this.addMapLoaderEventListeners();
         }
      }
      ...
      private function generateMap(param1:XML) : void
      {
         ...
         {
            _loc3_ = new AvatarAssetDownloadLibrary(_loc2_.@id,_loc2_.@revision,this.var_2265,this._assets);
      ...
      public function loadFigureSetData(param1:IAvatarFigureContainer, param2:IAvatarImageListener) : void
      {
         ...
         if(_loc4_.length > 0)
         {
            ...
            {
               _loc6_ = this.var_491[_loc3_];
               if(_loc6_ == null)
               {
                  _loc6_ = [];
               }
               _loc6_.push(param2);
               this.var_491[_loc3_] = _loc6_;
            }
            this.var_909[_loc3_] = _loc4_;
      ...
      private function libraryComplete(param1:Event) : void
      {
         ...
         var _loc10_:* = null;
         var _loc2_:* = [];
         for(_loc3_ in this.var_909)
         {
            _loc7_ = true;
            _loc4_ = this.var_909[_loc3_];
      ...
      {
         var _loc1_:* = null;
         while(this.var_730.length > 0 && this.var_729.length < this.const_1672)
         {
            _loc1_ = this.var_730[0];
            _loc1_.startDownloading();
            this.var_729.push(this.var_730.shift());
         }
      }
      
      private function addToQueue(param1:AvatarAssetDownloadLibrary) : void
      {
         if(!param1.isReady && this.var_730.indexOf(param1) == -1 && this.var_729.indexOf(param1) == -1)
         {
