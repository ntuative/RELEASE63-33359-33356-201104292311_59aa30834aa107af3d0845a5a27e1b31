package com.sulake.habbo.friendlist.domain
{
   ...
   public class FriendCategories
   {
         ...
         try
         {
            ...
            for each(updatedFriend in p.updatedFriends)
            {
               ...
               if(currentlyOnline && !updatedFriend.online)
               {
                  ...
                  this.var_463.notifications.addOfflineNotification(updatedFriend.name,updatedFriend.realName);
               }
               if(!currentlyOnline && updatedFriend.online)
               {
                  this.var_463.messenger.setOnlineStatus(updatedFriend.id,updatedFriend.online);
                  this.var_463.view.setNewMessageArrived();
                  this.var_463.notifications.addOnlineNotification(updatedFriend.name,updatedFriend.figure,updatedFriend.realName);
               }
      ...
      private function updateCategories(param1:Array) : void
      {
         ...
         this.findCategory(FriendCategory.const_440).received = true;
         this.findCategory(FriendCategory.const_1301).received = true;
