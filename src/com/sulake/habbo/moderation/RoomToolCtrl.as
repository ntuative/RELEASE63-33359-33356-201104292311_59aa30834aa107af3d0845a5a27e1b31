package com.sulake.habbo.moderation
{
   ...
   public class RoomToolCtrl implements IDisposable, TrackedWindow
   {
      ...
      private var var_560:ITextFieldWindow;
      
      private var var_645:Boolean = true;
      
      private var var_1050:ICheckBoxWindow;
      
      private var var_1591:ICheckBoxWindow;
      
      private var var_1592:ICheckBoxWindow;
      ...
      public function show() : void
      {
         ...
         this.var_34.connection.send(new GetModeratorRoomInfoMessageComposer(this.var_335));
         Logger.log("BEGINNING TO SHOW: " + this.var_335);
      }
      
      public function getType() : int
      {
         return WindowTracker.const_1633;
      }
      ...
      {
         Logger.log("GOT ROOM INFO: " + param1.flatId + ", " + this.var_335);
         if(param1.flatId != this.var_335)
         {
            Logger.log("NOT THE SAME FLAT: " + param1.flatId + ", " + this.var_335);
            return;
         }
         this._data = param1;
         this.populate();
      ...
      public function populate() : void
      {
         ...
         this.var_560 = ITextFieldWindow(this._frame.findChildByName("message_input"));
         this.var_560.procedure = this.onInputClick;
         this.var_1217 = IDropMenuWindow(this._frame.findChildByName("msgTemplatesSelect"));
         this.prepareMsgSelect(this.var_1217);
         this.var_1217.procedure = this.onSelectTemplate;
         this.var_1050 = ICheckBoxWindow(this._frame.findChildByName("kick_check"));
         this.var_1591 = ICheckBoxWindow(this._frame.findChildByName("lock_check"));
         this.var_1592 = ICheckBoxWindow(this._frame.findChildByName("changename_check"));
      ...
      private function onChatlog(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         this.var_34.windowTracker.show(new ChatlogCtrl(new GetRoomChatlogMessageComposer(0,this._data.flatId),this.var_34,WindowTracker.const_838,this._data.flatId),this._frame,false,false,true);
      }
      ...
      private function method_9(param1:Boolean) : void
      {
         if(this.var_645 || this.var_560.text == "")
         {
            this.var_34.windowManager.alert("Alert","You must input a message to the user",0,this.onAlertClose);
            return;
         }
         var _loc2_:int = this.determineAction(param1,this.var_1050.isSelected);
         this.var_34.connection.send(new ModeratorActionMessageComposer(ModeratorActionMessageComposer.const_1636,_loc2_,this.var_560.text,"",""));
         if(this.var_1591.isSelected || this.var_1592.isSelected || this.var_1050.isSelected)
         {
            this.var_34.connection.send(new ModerateRoomMessageComposer(this._data.flatId,this.var_1591.isSelected,this.var_1592.isSelected,this.var_1050.isSelected));
         }
         this.dispose();
      }
      ...
      private function determineAction(param1:Boolean, param2:Boolean) : int
      {
         if(param2)
         {
            return !!param1 ? 0 : int(ModeratorActionMessageComposer.const_1756);
         }
         return !!param1 ? 0 : int(ModeratorActionMessageComposer.const_1737);
      }
      ...
      private function onSelectTemplate(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         var _loc3_:String = this.var_34.initMsg.roomMessageTemplates[this.var_1217.selection];
         if(_loc3_ != null)
         {
            this.var_645 = false;
            this.var_560.text = _loc3_;
         }
      }
   }
}
