package com.sulake.habbo.widget.memenu
{
   ...
   public class EffectView
   {
      ...
      private const const_2161:int = 1000;
      ...
      private var var_965:IWindow;
      
      public function EffectView()
      {
         super();
         this.var_209 = new Timer(this.const_2161);
      ...
      private function onUpdate(param1:Event = null) : void
      {
         ...
         if(this.var_182.isActive)
         {
            _loc2_ = this.var_182.secondsLeft / Number(this.var_182.duration);
            this.var_966.width = _loc2_ * this.var_324;
         }
         else
         {
            this.var_966.width = 0;
            this.var_209.stop();
         }
         this.setTimeLeft();
      }
      ...
      private function setTimeLeft() : void
      {
         ...
         {
            this.var_784 = this._window.findChildByName("time_left") as ITextWindow;
            if(this.var_784 == null)
            {
               return;
            }
         ...
         if(!this.var_182.isActive)
         {
            this.var_784.caption = "${widgets.memenu.effects.activate}";
            return;
         }
         this.var_784.caption = "${widgets.memenu.effects.active.timeleft}";
         var _loc1_:String = this.var_784.text;
         var _loc2_:int = this.var_182.secondsLeft;
      ...
      private function createWindow(param1:String) : void
      {
         ...
         var _loc7_:* = null;
         var _loc3_:String = "";
         if(this.var_182.isInUse)
         {
            _loc3_ = "memenu_effect_selected";
         }
         else if(this.var_182.isActive)
         {
         ...
         else
         {
            ...
            this.var_965 = this._window.findChildByName("effect_hilite");
            this.var_965.visible = false;
         }
         this.setTimeLeft();
         this.var_966 = this._window.findChildByName("loader_bar");
         if(this.var_966 != null)
         {
            this.var_324 = this.var_966.width;
            this.var_209.start();
            this.onUpdate();
         }
         if(this.var_182.icon)
         {
            this.setElementBitmap("effect_icon",this.var_182.icon);
         }
      }
